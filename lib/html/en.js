...
function changeDates(id) {
	'use strict';
	var i, text1, text2, dates, date, newdate, formatter, pipe, radioElement;

	radioElement = getCheckedRadio('pipeformat');
	if (!radioElement) {
		console.warn("Radio button 'pipeformat' not found.");
		return;
	}
	pipe = radioElement.value;
	...
	for (i = 0; i < dates.length; i = i + 1) {
		...
		formatter = /** @type {Object<string, *>} */(window)[id];
...
function onDateChange() {
	...
	if (!checkedDate) {
		console.warn("Radio button 'dateformat' not found.");
		return;
	}
	var dateId = checkedDate.id;
	changeDates(dateId);
	localStorage.setItem("datefmt", dateId);
	window.updateURL('dateformat', checkedDate.value);
}
...
function onPipeChange() {
	...
	window.updateURL('pipeformat', checkedPipe.value);
}
